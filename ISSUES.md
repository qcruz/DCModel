...
| Born rule for position | **T3 (Cycle 338 update — Step 6a T1 NEW):** Derivation chain V(φ)→Schrödinger→⟨ε⟩∝|ψ|² is T2a (Steps 1-5). **NEW Step 6a [T1, C338]:** V(φ) nonlinear term V'''(φ₀)=6βφ₀ generates a slow source S(x)=3βφ₀(φ_c²/2)|ψ(x)|² after time-averaging σ². KEY IDENTITY: S(x)/⟨ε(x)⟩ = 3βφ₀/ω_c² = 3βφ₀/(2α) = κ_NL = 0.17425 [T1 EXACT, 16/16 PASS, `equations/born_rule_d3_coupling.py`]. This constant κ_NL is determined by V(φ) alone with no free parameters. S(x) = κ_NL × ⟨ε(x)⟩ holds for ANY wave function ψ(x) — verified for cos(kx), Gaussians, sech(x), wave packets. **Step 6 decomposed:** Step 6a T1 (S∝⟨ε⟩ from V(φ)); Step 6b T3 remaining gap (D3 localization rate ∝ |S(x)| — structural argument, not yet derived from V(φ)). Born rule overall T3 (unchanged — Step 6b still open). Path to T2a: derive Step 6b from the field equation dynamics at D3 depth. | `measurement.md`, `equations/born_rule_derivation.py`, `equations/born_rule_schrodinger.py`, `equations/born_rule_d3_coupling.py` | Derive D3 localization rate ∝ |S(x)| from V(φ) (Step 6b → T2a closes Born rule) |
