...
| Born rule for position | **T3 (Cycle 336 update):** Derivation chain V(φ)→Schrödinger→⟨ε⟩∝|ψ|² now T2a (Steps 1-5 all T1). KEY T1: exact cancellation -ω_c²+2α=0 → Schrödinger equation i∂_tψ=-(1/2ω_c)∂²_xψ falls out of V(φ) without postulation; ⟨ε(x)⟩=(φ_c²ω_c²/2)|ψ(x)|² [T1]; norm conservation Hermitian [T1]. Module: `equations/born_rule_schrodinger.py` (11/11 PASS). Born rule remains T3 (limited by Step 6: D3 coupling to energy density). Remaining T4: derive Step 6 from V(φ) explicitly. | `measurement.md`, `equations/born_rule_derivation.py`, `equations/born_rule_schrodinger.py` | Derive D3 localization coupling rate ∝ ⟨ε(x)⟩ from V(φ) (Step 6 → T2a closes Born rule) |
