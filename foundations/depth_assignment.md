...
> The primary bottleneck — why U(1) closes at D5, SU(2) at D6, SU(3) at D7 — has been
> mapped and largely resolved through a multi-step derivation chain.
>
> **Result:** Three structural constraints narrow the possible assignments significantly.
> The second half of Bottleneck 1 is proved: n coincident degenerate zero modes →
> configuration space S^(2n-1) → SU(n) gauge group. The DOF count mechanism is
> established.
>
> Scalar inter-depth coupling lifts zero modes; gauge (derivative) coupling preserves
> n independent zero modes. The D5 half-vortex background makes the D6 kink complex
> (W=−1/2, ∫j_x = −2π/(5ξ) exact). Structural chain complete.
>
> Key progress on why 2n real DOFs arise per n kinks: the substrate field equation is
> second-order in time → each zero mode has 2 real DOFs (position q and velocity v).
> For n kinks: 2n real DOFs → S^(2n-1). Real isometry SO(2n). At D5 (n=1):
> SO(2) = U(1) exactly — no complex structure needed (Tier 2 candidate). At D6/D7:
> SO(4)/SO(6) too large; D5 complex structure J reduces to U(2)/U(3) → SU(2)/SU(3).
> Verified numerically in `equations/u1_from_paired_modes.py`.
>
...
is 2n real DOFs → S^(2n-1) ⊂ ℝ^(2n) (`foundations/complex_zero_mode_gap.md`).
The real isometry SO(2n) is reduced to U(n) by the D5 complex structure.

NOTE: The formula γ_D = (16/3)√β cited here in prior versions was RETRACTED
...
1. **Formally prove that D5 U(1) defines J on D6/D7 zero mode spaces.** (Remaining
   Tier 3 gap.) The structural argument is: D6 modes carry U(1) charge
   ...
   `equations/complex_structure_derivation.py` to verify J² = −I from the
   D5 gauge background on the D6 mode space.

2. **Derive the 2 real DOFs per zero mode from the substrate equation.** (Largely
   resolved: second-order PDE → zero mode initial data is (q,v) ∈ ℝ².)
...
4. **Rule out alternative assignments.** Formally show that U(1) cannot close at D6
   (the D6 bifurcation opens 4 real DOFs → SO(4), not SO(2) = U(1)). This is now
   structural: D6 has 4 real DOFs → SO(4) dim=6 ≠ 1 = dim(U(1)).
...
5. **Connect to Route B via substrate field equations.** Show that the substrate's
   ...
   thresholds — the codimension-1 argument gives 1 new mode per threshold,
   so n thresholds → n zero modes. Combined with the 2-DOF pairing, this
...
- `foundations/bifurcation_dynamics.md` — E_kink/E_total = 8/3 exact (γ_D retracted)
...
- `foundations/complex_zero_mode_gap.md` — 2 real DOFs per mode → SO(2n) → U(n) → SU(n)
- `foundations/zero_mode_multiplet.md` — n complex modes → SU(n) proved
- `foundations/bifurcation_mode_count.md` — full Bottleneck 1 structural chain
- `equations/u1_from_paired_modes.py` — numerical verification SO(2n) dim, U(n) commutant
