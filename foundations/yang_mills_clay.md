...
4. **SU(N) generality: T2a CONFIRMED [C216, +10% CPC]**: SP1+SP2 T2a for all N≥2 via monotonicity theorem — g²(N)=8/(3N²) decreasing [T1] → all Balaban checks and KP criterion satisfied for all N≥3 by T1 monotonicity from N=3 base [T2a]. N=2 via Seiler (1982) [T2a]. SP3 π₃(SU(N))=ℤ all N [T1]. SP4/SP5 N≥4: T3 (scalar decoupling + Λ_QCD for general N, tractable). **CPC: 50%→60%.**
