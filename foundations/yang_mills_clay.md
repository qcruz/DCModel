...
| C214 | **JW3c-a worldvolume Poincaré covariance T2a**: ym_poincare_covariance.py (new) — 7-part verification. Part A: Z₂ kink antisymmetry residual 0.00e+00 [T1]; BPS equation φ' = W'(φ) residual 2.14e-05 [T1 given ξ]; T^{μν} = σ η^{μν} tension=112.0 M_Pl, pressure −3.88e-14≈0 [T1]. Part B: φ_kink(y) depends only on y [T1]; transverse ISO(3,1) unbroken algebraically [T1]. Part C: A_μ^a = (1/g)∂_μθ^a is Lorentz 4-vector; null wave boost residual 1.11e-16 [T1]. Part D: F^{μν}F_{μν} Lorentz invariant (residual 5.51e-16 [T1]; 3 frames tested). Part E: Poincaré algebra closes — [L_01,L_12]=L_02 residual 0.00e+00, [L_01,L_03]=+L_13 residual 0.00e+00 [T1]; all 6 commutators exact. Part F: c_eff = 1 (substrate propagation isotropic for domain-wall parallel modes) [T1]. Part G: JW3c two-tier decomposition — JW3c-a (worldvolume ISO(3,1) given flat substrate) **T2a**; JW3c-b (spacetime emergence D3/D4 → Minkowski signature) **T3 (open)**; JW3c overall T3 (strengthened — gap precisely characterized). ISSUES.md T4 updated: A_μ^a Lorentz 4-vector T1 → JR zero mode is 4D Dirac spinor T2a conditional on JW3c-a. strong_force.md audited: SP2 T2a [C212], Clay 74%, remaining gaps updated. educational/07_open_questions.md Gap 1: JW3c-a T2a added, JW criterion table added, spacetime emergence gap precisely characterized. Clay: ~74% (unchanged — JW3c overall still T3). CPC: ~50% (unchanged). | **~74%** | **~50%** |
...
**All SP1–SP5 reached T2a.** SP1 T2a [C203], SP2 T2a [C212], SP3 T2a [C187], SP4 T2a [C184], SP5 T2a [C191/C197]. CPC: ~50%.

**JW criteria documentation [C213]**: ym_clay_requirements.py maps 6/7 criteria T2a; JW3c (Poincaré) T3 remaining. This is the first formal Jaffe-Witten checklist for the DFC construction.

**Next priorities (by impact on CPC):**
1. **JW3c-b spacetime emergence T3→T2a [C214: JW3c-a T2a established]**: JW3c-a (worldvolume ISO(3,1) given flat substrate) is now T2a [C214, all checks T1]. Residual gap JW3c-b: formally show that the D3/D4 depth localization behavior produces the Minkowski metric η_μν — i.e., that spacetime signature (−,+,+,+) is a structural consequence of the substrate's D4 inertia behavior, not an assumption. Potential route: Noether current for D4 time-translation symmetry → energy sign → Minkowski signature derivation.
2. **SP2 BPS Hamiltonian form T3→T2a**: Formal 4D bound H ≥ I₄×Q̂_top×m from BPS→quantum chain. The gap existence is T2a; the form H ≥ Δ×(I−P_Ω) [where P_Ω=|Ω⟩⟨Ω|] is what's needed.
3. **SP5 S10 T2b→T2a**: C_match correction 0.34% — show this comes from 2-loop threshold (not free parameter). Currently α_s(M_Z) = 0.11566 (−2.15%, T2a) from V(φ); closing to T2a requires matching C_match to 0.34% accuracy.
4. **SU(N) generality (+10% CPC)**: DFC derives SU(3) specifically; Clay requires "any" gauge group. Generalizing the D7=SU(N) argument to arbitrary N would unlock +10% CPC.
