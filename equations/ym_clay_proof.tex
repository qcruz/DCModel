...
\begin{remark}[Selection mechanism, not numerology]\label{rem:selection}
The identity $I_4 = C_2(\mathrm{fund},\mathrm{SU}(3)) = 4/3$ is not numerology.
The two quantities on either side are determined by entirely independent means:
\begin{enumerate}
\item $I_4 = \int_{-\infty}^{+\infty}\mathrm{sech}^4(u)\,du = 4/3$ is computed
  from the kink profile of $V(\phi)$ alone, by antiderivative
  $[\tanh u - \tfrac{1}{3}\tanh^3 u]_{-\infty}^{+\infty}$.
  No Lie group, no representation theory, and no knowledge of $n$ enters this
  computation.  The value $4/3$ is fixed by $V(\phi)$.

\item $C_2(\mathrm{fund},\mathrm{SU}(n)) = (n^2-1)/(2n)$ is a function of $n$
  only, determined by Lie algebra theory.  Setting it equal to $4/3$ gives the
  polynomial equation $3n^2 - 8n - 3 = 0$ with discriminant $= 100 = 10^2$
  (rational arithmetic, T1).  The unique positive integer solution is $n = 3$;
  no other positive integer satisfies $C_2 = 4/3$.
\end{enumerate}
The meeting of these two independent facts — one from $V(\phi)$ dynamics, one from
representation theory — uniquely selects $\mathrm{SU}(3)$.  This is a selection
mechanism: $V(\phi)$ determines $I_4$; $I_4$ forces $n$; $n$ determines $G$.
\end{remark}
