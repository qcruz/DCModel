...
\textit{(Assumption~A:} The kink zero-mode couples to the \emph{fundamental}
representation of $\mathrm{SU}(n)$, not the adjoint or higher representations.
This is supported by Jackiw-Rebbi holonomy analysis \cite[C307--C314]{DFC}
(triality $t=1$) but is stated here as an explicit hypothesis.\textit{)}

\textit{Step 3: Unique termination at $n = 3$ (under Assumption~A).}
